...
sudo /opt/td-agent/bin/fluent-gem install --version 0.14.0 fluent-plugin-cloudwatch-logs
sudo /opt/td-agent/bin/fluent-gem install --version 2.5.0 fluent-plugin-concat
sudo /opt/td-agent/bin/fluent-gem install --version 2.6.2 fluent-plugin-grok-parser
sudo /opt/td-agent/bin/fluent-gem install --version 1.0.2 fluent-plugin-json-in-json-2
#sudo /opt/td-agent/embedded/bin/fluent-gem install --version 2.4.0 fluent-plugin-kubernetes_metadata_filter
sudo /opt/td-agent/bin/fluent-gem install --version 1.0.0 fluent-plugin-multi-format-parser
sudo /opt/td-agent/bin/fluent-gem install --version 2.0.1 fluent-plugin-prometheus
sudo /opt/td-agent/bin/fluent-gem install --version 2.1.0 fluent-plugin-record-modifier
sudo /opt/td-agent/bin/fluent-gem install --version 2.4.0 fluent-plugin-rewrite-tag-filter
sudo /opt/td-agent/bin/fluent-gem install --version 1.0.5 fluent-plugin-systemd
sudo apt-get remove -y g++ make
sudo mv /tmp/aws-fluentd-cell.conf /etc/td-agent/td-agent.conf
# verify fluentd config
sudo REGION=us-east-1 CLUSTER_NAME=dummy /opt/td-agent/bin/fluentd --dry-run -c /etc/td-agent/td-agent.conf
sudo systemctl daemon-reload
